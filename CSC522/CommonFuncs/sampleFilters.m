function [h, array] = display_network(A, opt_normalize, opt_graycolor, opt_colmajor)
...
if isa(A,'cell')
    ...
    B = zeros(28*28,s);
    for i = 1:s
        B(:,i) = reshape(A{v(i)},28*28,1);
    end
    A = B;
elseif ismatrix(A)
    if (size(A,2) > 204)
        randsel = randi(size(A,2),204,1);
        %randsel = 1:204;
        A = A(:,randsel);
    end
else
    disp('Error, bad type for A');
    return;
end
...
array=-ones(buf*(sz+buf),buf+n*(sz+buf));
